...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      `📅 Fetching daily prompts from ${today.toISOString()} to ${tomorrow.toISOString()}`
    );
    ...
    for (const collectionName of collectionsToCount) {
      try {
        ...
        console.log(
          `📊 Collection ${collectionName}: ${todayCount} prompts today`
        );
        dailyPrompts += todayCount;

        // Calculate water usage for today's searches
        const todaySearches = await collection
          .find({
            createdAt: {
              $gte: today,
              $lt: tomorrow,
    ...
    console.log(
      `✅ Daily prompts found: ${dailyPrompts}, Daily water usage: ${dailyWaterUsage}mL`
    );
