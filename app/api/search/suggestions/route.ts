...
async function getGoogleSuggestions(query: string, limit: number = 8): Promise<string[]> {
  try {
...
async function getDuckDuckGoSuggestions(query: string, limit: number = 5): Promise<string[]> {
  try {
    const response = await fetch(
      `https://api.duckduckgo.com/?q=${encodeURIComponent(query)}&format=json&pretty=1&no_html=1`,
      {
        signal: AbortSignal.timeout(5000)
      }
    );
...
async function getGroqAISuggestions(query: string, limit: number = 6): Promise<string[]> {
  try {
    ...
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a search autocomplete system. Generate relevant search query completions for the partial query. Return only a simple JSON array of 4-6 completion strings. No explanations, just the array.'
          },
          {
            role: 'user',
    ...
    if (content) {
      try {
        ...
        if (Array.isArray(suggestions)) {
          return suggestions
            .slice(0, limit)
            .filter((s: any) => typeof s === 'string' && s.trim().length > 0 && s.length < 100);
        }
      } catch (parseError) {
    ...
    console.warn('Groq AI API error:', error instanceof Error ? error.message : String(error));
    return [];
  }
}

// Wikipedia API - For factual query completions
async function getWikipediaSuggestions(query: string, limit: number = 4): Promise<string[]> {
  try {
    const response = await fetch(
...
async function getLiveSuggestions(query: string, limit: number = 8): Promise<string[]> {
  ...
  try {
    ...
    if (uniqueSuggestions.length > 0) {
      console.log(`✅ Live suggestions: ${uniqueSuggestions.length} from ${apiSuccessCount} APIs for "${query}"`);
    }
...
function getFallbackSuggestions(query: string, limit: number = 8): string[] {
  ...
  } else if (queryLower.includes('what is') || queryLower.startsWith('what is')) {
    fallbacks.push(
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (suggestions.length < Math.floor(limit / 2)) {
      const popularSearches = await getPopularSearches(limit - suggestions.length);
