...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      const googleApiKey =
        process.env.GOOGLE_API_KEY || "AIzaSyBqK7VgYpE3tO5m9Y8v6qXrP2fD4cH9jL1";
      ...
      const googleResponse = await fetch(
        `https://www.googleapis.com/customsearch/v1?key=${googleApiKey}&cx=${searchEngineId}&q=${encodeURIComponent(
          query
        )}&searchType=image&num=6&safe=active&imgSize=large`,
        {
          headers: {
      ...
      if (googleResponse.ok) {
        ...
        console.log(
          `📊 Google returned ${googleData.items?.length || 0} images`
        );
        ...
        if (googleData.items && googleData.items.length > 0) {
          ...
          console.log(
            `✅ Google API success: ${images.length} real images found`
          );
          ...
          return NextResponse.json({
            ...
            images: images,
            total:
              parseInt(googleData.searchInformation?.totalResults) ||
              images.length,
            source: "Google Images",
        ...
        console.log(
          `❌ Google API error: ${googleResponse.status} - ${errorText}`
        );
      }
    } catch (googleError) {
    ...
    try {
      ...
      const serpResponse = await fetch(
        `https://serpapi.com/search.json?engine=google_images&q=${encodeURIComponent(
          query
        )}&api_key=${serpApiKey}&num=6&safe=active&tbm=isch`,
        {
          headers: {
            "User-Agent": "Alchemist-AI/1.0",
          },
      ...
      if (serpResponse.ok) {
        ...
        console.log(
          `📊 SerpAPI returned ${serpData.images_results?.length || 0} images`
        );
    ...
    try {
      ...
      const bingResponse = await fetch(
        `https://api.bing.microsoft.com/v7.0/images/search?q=${encodeURIComponent(
          query
        )}&count=6&safeSearch=Moderate&imageType=Photo`,
        {
          headers: {
            "Ocp-Apim-Subscription-Key":
              process.env.BING_SEARCH_API_KEY || "demo-key",
            "User-Agent": "Alchemist-AI/1.0",
          },
        }
      );
      ...
      if (bingResponse.ok) {
        ...
        if (bingData.value && bingData.value.length > 0) {
          ...
          console.log(
            `✅ Bing API success: ${images.length} real images found`
          );
    ...
    try {
      ...
      for (let i = 0; i < Math.min(6, searchVariations.length * 2); i++) {
        ...
        images.push({
          ...
          thumbnail: `https://source.unsplash.com/300x200/?${encodeURIComponent(
            searchTerm
          )}&sig=${i}`,
          alt: `${searchTerm} - Image related to ${query}`,
          source: "Unsplash",
          photographer: "Unsplash Contributors",
          link: `https://unsplash.com/s/photos/${encodeURIComponent(
            searchTerm
          )}`,
        });
      }
      ...
      console.log(
        `✅ Generated ${images.length} Unsplash images for "${query}"`
      );
    ...
    try {
      ...
      const wikimediaResponse = await fetch(
        `https://commons.wikimedia.org/w/api.php?action=query&format=json&list=search&srsearch=${encodeURIComponent(
          query
        )}&srnamespace=6&srlimit=6&origin=*`,
        {
          headers: {
            "User-Agent": "Alchemist-AI/1.0",
          },
      ...
      if (wikimediaResponse.ok) {
        ...
        if (
          ...
          const images = await Promise.all(
            wikimediaData.query.search
              ...
              .map(async (item: any, index: number) => {
                ...
                const thumbnailUrl = `https://commons.wikimedia.org/wiki/Special:FilePath/${encodeURIComponent(
                  filename
                )}?width=300`;
                ...
                return {
                  ...
                  link: `https://commons.wikimedia.org/wiki/File:${encodeURIComponent(
                    filename
                  )}`,
                };
              })
          ...
          console.log(
            `✅ Wikimedia API success: ${images.length} images found`
          );
    ...
    try {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        duckduckgoImages.push({
          ...
          link: `https://duckduckgo.com/?q=${encodeURIComponent(
            query
      ...
      console.log(
        `✅ Generated ${duckduckgoImages.length} search-based images for "${query}"`
      );
