...
async function testCalendarData() {
  const client = new MongoClient('mongodb+srv://shahhaseebahmadkhan9_db_user:1234Dvorak$@cluster0.igtsejk.mongodb.net/');
  ...
  try {
    ...
    if (searchesCount > 0) {
      ...
      console.log('📝 Sample search document:', JSON.stringify(sampleSearch, null, 2));
      ...
      const yearlyStats = await db.collection('searches').aggregate([
        {
          $match: {
      ...
      console.log('📊 Yearly stats result:', JSON.stringify(yearlyStats, null, 2));
      
      // Test daily aggregation
      const dailyUsage = await db.collection('searches').aggregate([
        {
          $match: {
            userId: new ObjectId(userId),
            createdAt: { $gte: yearStart, $lte: yearEnd }
      ...
      console.log('📈 Daily usage result:', JSON.stringify(dailyUsage, null, 2));
    }
